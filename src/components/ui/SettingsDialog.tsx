...
type SettingsTab = 'system' | 'sync' | 'export' | 'ai' | 'design' | 'about'
      ...
      >
        ...
        >
          <TabsList className="flex h-full w-[140px] shrink-0 flex-col items-stretch justify-start gap-1 rounded-none border-r border-line-default bg-surface-panel-alt p-3 text-fg-secondary">
            {([['system','系统设置'],['sync','同步设置'],['export','导入导出'],['ai','AI 设置'],['design','设计系统'],['about','关于']] as const).map(([val, label]) => (
              <TabsTrigger
                key={val}
                value={val}
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            <TabsContent value="about" className="flex-1 overflow-y-auto p-6 scrollbar-hide mt-0">
              <AboutSettings />
            </TabsContent>
          </div>
        </Tabs>
