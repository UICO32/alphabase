...
function renderBlock(block: Record<string, unknown>): string {
  ...
  switch (type) {
    case 'heading': {
      ...
      return `<h${level} style="font-size:${sizes[level] || '1.1em'};font-weight:${weights[level] || 600};${BLOCK_STYLE}">${renderedContent}</h${level}>${renderedChildren}`
    }
    case 'paragraph':
      return `<p style="${BLOCK_STYLE}">${renderedContent || '<br />'}</p>${renderedChildren}`
    case 'bulletListItem':
      return `<li style="${BLOCK_STYLE};list-style-type:disc">${renderedContent}</li>${renderedChildren}`
    case 'numberedListItem':
      return `<li style="${BLOCK_STYLE}">${renderedContent}</li>${renderedChildren}`
    case 'checkListItem':
      return `<li style="${BLOCK_STYLE};list-style-type:none;display:flex;align-items:flex-start;gap:4px"><input type="checkbox" ${props?.checked ? 'checked' : ''} disabled style="margin-top:3px;flex-shrink:0" /><span>${renderedContent || '<br />'}</span></li>${renderedChildren}`
    case 'image': {
      ...
      const img = url
        ...
        : ''
      const cap = caption ? `<p style="font-size:0.85em;opacity:0.6;${BLOCK_STYLE}">${escapeHTML(caption)}</p>` : ''
      return `<div style="${BLOCK_STYLE}">${img}${cap}</div>${renderedChildren}`
    }
    default:
      return renderedContent
        ? `<div style="${BLOCK_STYLE}">${renderedContent}</div>${renderedChildren}`
        : renderedChildren
  }
}
...
function renderInlineContent(node: Record<string, unknown>): string {
  if (node.type === 'text') {
    ...
    if (styles.code) text = `<code style="font-size:0.875em;background:rgba(0,0,0,0.06);border-radius:3px;padding:0 3px;font-family:ui-monospace,monospace">${text}</code>`
    if (styles.underline) text = `<u>${text}</u>`
    return text
  }
  if (node.type === 'link') {
    const href = escapeHTML((node.href as string) || '')
    const linkContent = (node.content as unknown[])?.map((c) => renderInlineContent(c as Record<string, unknown>)).join('') || href
